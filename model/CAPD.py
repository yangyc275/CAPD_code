...
from modules import ModifiedResnet, PointNet2MSG, PoseNet, DeepPriorDeformer
from losses import SmoothL1Dis, ChamferDis, PoseDis, l2_loss
...
class Net(nn.Module):
    def __init__(self, nclass=6, nprior=1024, use_CrossAtt=False):
        ...
        self.posenet = PoseNet(nclass, nprior, use_CrossAtt)
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            A1, Qv1, r1, t1, s1 = self.posenet(rgb_local, pts1_local, prior_local, pts1, prior, index)
            ...
            A2, Qv2, r2, t2, s2 = self.posenet(rgb_local, pts2_local, prior_local, pts2, prior, index)
            ...
            A, Qv, r, t, s = self.posenet(rgb_local, pts_local, prior_local, pts, prior, index)
